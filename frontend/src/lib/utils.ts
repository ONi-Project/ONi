...
export function numberDisplayConvert(number: number) {
  ...
  const grade = (digit - digit % 3) / 3
  if (number < 10000) return String(number)
  if (digit % 3 == 0) return String((number / Math.pow(10, digit)).toFixed(1) + unitList[grade])
...
export function timePassedDisplayConvert(timestamp: number) {
  ...
  const month = 30 * day
  const year = 365 * day
  if (timestamp == 0) return "无数据"
  else if (passed < 60 * 1000) return "刚刚"
  else if (passed < hour) return Math.floor(passed / minute) + " 分钟前"
  else if (passed < day) return Math.floor(passed / hour) + " 小时前"
  else if (passed < month) return Math.floor(passed / day) + " 天前"
  else if (passed < year) return Math.floor(passed / month) + " 月前"
  else return "很久以前"
}
...
export function timeLengthDisplayConvert(timestamp: number) {
  ...
  else if (passed < hour) return Math.floor(passed / minute) + " 分钟"
  else if (passed < day) return Math.floor(passed / hour) + " 小时"
  else if (passed < month) return Math.floor(passed / day) + " 天"
  else if (passed < year) return Math.floor(passed / month) + " 月"
  else return "很久"
}
